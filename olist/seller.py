...
class Seller:
    ...
    def get_review_score(self):
        ...
        review_scores = self.data['order_reviews'][['order_id', 'review_score']]\
            .merge(self.data['orders'][['order_id']], on='order_id')\
            .merge(self.data['order_items'][['order_id', 'seller_id']], on='order_id')

        avg_score = pd.DataFrame(review_scores.groupby('seller_id')['review_score'].mean())
        scores = review_scores.groupby('seller_id')['review_score'].value_counts().unstack(level=-1)
        scores['all'] = scores.sum(axis=1)
        scores['share_of_five_stars'] = scores[5] / scores['all']
        scores['share_of_one_stars'] = scores[1] / scores['all']
        return scores[['share_of_five_stars', 'share_of_one_stars']].join(avg_score).fillna(0).reset_index()
